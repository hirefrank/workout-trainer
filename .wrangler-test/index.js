...
__name(deleteCookieHeader, "deleteCookieHeader");
...
var program_default = programData;

// src/lib/schemas.ts
var LoginSchema = external_exports.object({
  password: external_exports.string().min(1).max(100)
});
var WorkoutQuerySchema = external_exports.object({
  week: external_exports.number().int().min(1).max(programData.program.weeks),
  day: external_exports.number().int().min(1).max(7)
});
var WorkoutCompletionWithNotesSchema = external_exports.object({
  week: external_exports.number().int().min(1).max(programData.program.weeks),
  day: external_exports.number().int().min(1).max(7),
  notes: external_exports.string().max(500).optional()
});
var PushSubscriptionSchema = external_exports.object({
  endpoint: external_exports.string().url(),
  expirationTime: external_exports.number().nullable().optional(),
  keys: external_exports.object({
    p256dh: external_exports.string(),
...
function htmlLayout(content, title, footerStats, description, programMeta) {
...
<head>
  ...
  <link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><text y='.9em' font-size='90'>\u{1F4AA}</text></svg>">
      ...
      <footer class="mt-8 pt-4 border-t-4 border-black">
        <div class="flex flex-col sm:flex-row justify-between items-center gap-2">
          <p class="text-xs font-bold tracking-wider uppercase">
            ${footerStats ? `COMPLETED ${footerStats.completed} OUT OF ${footerStats.total} WORKOUTS` : programMeta ? `${programMeta.weeks} WEEKS | ${programMeta.daysPerWeek} DAYS/WEEK` : "WORKOUT TRACKER"}
          </p>
...
__name(htmlLayout, "htmlLayout");

// src/templates/components.ts
function workoutCard(week, day, exercises, isComplete, canEdit, completionNotes) {
  const completeClass = isComplete ? "bg-green-100" : "";
  const buttonClass = isComplete ? "bg-white hover:bg-zinc-100" : "bg-green-400 hover:bg-green-500";
      ...
      <div class="flex items-start justify-between gap-4">
        <div class="flex-1">
          <div class="flex items-center gap-2">
            ...
            ${isComplete ? '<svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>' : ""}
          </div>
          <p class="text-sm text-zinc-600 mt-1">${day.exercises.length} exercises</p>
          ${isComplete && completionNotes ? `<p class="text-sm text-zinc-700 mt-2 italic bg-zinc-50 p-2 border border-zinc-300">\u{1F4DD} ${escapeHtml(completionNotes)}</p>` : ""}
        </div>
...
__name(workoutCard, "workoutCard");
function exerciseRow(exercise, exerciseData) {
  let weight = "";
  if (exercise.weight) {
  ...
  const setsReps = parts.length > 0 ? parts.join(" \xD7 ") : "\u2014";
  const weightTypeLabel = exercise.weight_type ? `<span class="ml-2 text-xs uppercase text-zinc-500">(${escapeHtml(
...
__name(exerciseRow, "exerciseRow");
...
async function handleDashboard(request, env) {
  ...
  if (isNaN(currentWeek) || currentWeek < 1 || currentWeek > program_default.program.weeks) {
    return new Response(
      htmlLayout(
        ...
        void 0,
        void 0,
        { weeks: program_default.program.weeks, daysPerWeek: program_default.program.days_per_week }
      ),
      {
        status: 404,
  ...
  const week = program_default.weeks.find((w) => w.number === currentWeek);
  if (!week) {
    return new Response(
      htmlLayout(
        `<div class="text-center py-12">
  ...
  const totalWeeks = program_default.program.weeks;
  const completedCount = week.days.filter(
    (day) => !!completions[`workout:${currentWeek}-${day.number}`]
  ).length;
  const totalWorkouts = program_default.weeks.reduce((sum, w) => sum + w.days.length, 0);
        ...
        <div class="flex items-center justify-between mb-4">
          <div>
            <h2 class="text-2xl font-bold">
              Week ${currentWeek} ${week.is_deload ? '<span class="text-red-600">\u{1F534} DELOAD</span>' : ""}
            </h2>
  ...
  return new Response(
    htmlLayout(
      content,
      `${program_default.program.name} - Week ${currentWeek}`,
      { completed: totalCompleted, total: totalWorkouts },
      program_default.program.description,
      { weeks: program_default.program.weeks, daysPerWeek: program_default.program.days_per_week }
    ),
    {
      headers: {
...
// src/handlers/workouts.ts
async function handleGetCompletions(request, env) {
  try {
    const { keys } = await env.WORKOUTS_KV.list({ prefix: "workout:" });
...
__name(handleGetCompletions, "handleGetCompletions");
async function handleMarkComplete(request, env) {
  try {
...
__name(handleMarkComplete, "handleMarkComplete");
async function handleUnmark(request, env) {
  try {
    if (!await isUserAuthenticated(request, env)) {
      return new Response(
        JSON.stringify({ error: "Unauthorized" }),
...
__name(handleUnmark, "handleUnmark");
